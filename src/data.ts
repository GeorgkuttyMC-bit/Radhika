export const personalInfo = {
  name: "Radhika Sudhakaran",
  title: "Project Manager | Marketing & Strategic Communications Expert",
  phone: "9400465583",
  email: "radhikajyothis@gmail.com",
  linkedin: "radhika-sudhakaran-b7263477",
  location: "Kakkanad, Ernakulam",
  about: "Experienced professional with a solid foundation in marketing, strategic communications, and client relationship management, and project management. Over 10 years of expertise in leading high-profile projects, managing multiple clients, and delivering results-driven campaigns for brands like Amazon, Logitech, Kochi Metro Rail Limited, and Paragon Footwear. Skilled in ensuring client requirements are met, projects are completed on time, and fostering strong client relationships."
};

export const coreCompetencies = [
  "Project Management and Coordination",
  "Marketing and Strategic Communication",
  "Client Relationship Management",
  "Budgeting and Resource Planning",
  "Deadline-Driven Execution"
];

export const keyAchievements = [
  "Successfully managed complex projects across multiple clients, ensuring on-time delivery and exceeding client expectations.",
  "Transitioned seamlessly between strategic communication and hands-on project management roles to meet diverse business needs.",
  "Led major campaigns for Kochi Metro Rail Limited and Paragon Footwear, achieving impactful brand milestones."
];

export const experience = [
  {
    role: "Account Director / Project Lead",
    company: "Bright Communications, Kochi",
    date: "Jun 2024 - Present",
    accounts: "Renai Medicity, Renai Kochi, KSINC, CIMAR Women’s Hospital, Vibha by Bharath Hospital, Aakash Educational Services",
    details: [
      "Directed end-to-end project management for marketing and branding initiatives, overseeing planning, execution, and delivery. (Both Offline & Online)",
      "Managed cross-functional teams to ensure seamless coordination and timely completion of projects.",
      "Acted as a strategic advisor to clients, providing insights to enhance project outcomes and achieve business goals.",
      "Balanced project deliverables with client relationship management to maintain long-term partnerships.",
      "Develop and implement marketing strategies and campaign plans to achieve business goals, ensuring adherence to KPIs."
    ]
  },
  {
    role: "Head of Brand Communications / Consultant",
    company: "Poptale integrated marketing Agency, Kochi",
    date: "Oct 2023 - May 2024",
    accounts: "Kitchen Treasures, Euro Guard, Kent constructions, Arakkal Gold & Diamonds",
    details: [
      "Led comprehensive branding and communications campaigns both offline and online."
    ]
  },
  {
    role: "Account Manager: Client Relations / Consultant",
    company: "Passio, Kochi",
    date: "Sept 2022 - June 2024",
    accounts: "Zafco group of companies, Trivandrum International School, Cochin international School",
    details: [
      "Supervised client-focused projects, ensuring alignment with business objectives and brand strategy. (Both Offline & Online)",
      "Coordinated project schedules, timelines, and resource allocation for optimal delivery.",
      "Collaborated with teams to design and implement communication strategies that drove project success."
    ]
  },
  {
    role: "Account Executive",
    company: "The Links India, Kochi",
    date: "Mar 2018 - Sept 2021",
    accounts: "Amazon, Logitech, Federal Bank, Novotel Kochi, Ibis",
    details: [
      "Gained a foundational understanding of branding and communications while contributing creatively to campaigns. (Both Offline & Online)",
      "Successfully managed campaigns for major clients, including Amazon and Logitech, enhancing brand visibility and customer engagement.",
      "Strategized and executed social media campaigns tailored to client requirements, resulting in increased online engagement."
    ]
  },
  {
    role: "Senior Client Service Executive",
    company: "Method Advertising, Kochi",
    date: "Sept 2016 - Feb 2018",
    accounts: "KMRL, Credai, Hedge, Asianet Broadband, Civil Supplies corporation, Daily Fish, Maruti Suzuki kerala region",
    details: [
      "Managed the branding and marketing activities for the inauguration of Kochi Metro Rail’s first phase.",
      "Created and executed various offline campaigns, ensuring impactful visibility and audience engagement.",
      "Recognized for delivering one of the most prominent projects of the year, enhancing the agency’s reputation."
    ]
  },
  {
    role: "Client Service Executive",
    company: "TeamOne Advertising, Kochi",
    date: "Aug 2014 - Sept 2015",
    accounts: "V Star, Eastern, Jayalakshmi, Muthoot",
    details: [
      "Emphasized how those clients helped shape foundational skills in branding and client management."
    ]
  }
];

export const education = [
  {
    degree: "MBA Advertising and Marketing Management (pursuing)",
    institution: "IGNOU, Kochi",
    year: "Ongoing"
  },
  {
    degree: "MFA in Advertising and Journalism",
    institution: "Amrita University, Kochi",
    year: "2013"
  }
];

export const awards = [
  {
    title: "Pepper Awards 2018",
    description: "KMRL Seed Calander"
  },
  {
    title: "Inkspell Awards 2019",
    description: "Amazon Seller stories"
  }
];

export const languages = ["English", "Malayalam"];
